![Snehal's banner](https://media.licdn.com/dms/image/v2/D5616AQHuKK0fOQ18pw/profile-displaybackgroundimage-shrink_350_1400/B56ZYzCjMSGcAY-/0/1744613035051?e=1750291200&v=beta&t=9IDI6gg359jHbPRiEP73pH9synawsbrLrh6OWWLqczE)

# 👾 Snehal Pawar
### 🕵️‍♀️ Cybersecurity Researcher | InfoSec Enthusiast | IT Engineer

---

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&duration=3000&pause=500&color=F70000&center=true&vCenter=true&width=435&lines=Security+Researcher;InfoSec+Analyst+in+training;Hacking+for+a+Safer+Tomorrow;Always+watching+the+logs..." alt="Typing SVG" />
</p>

---

### 🔍 Whoami
```bash
$ whoami
snehal_pawar
```

```bash
$ uname -a
CyberOS_Engr v3.0 | Focus: Network Security | Penetration Testing | Digital Forensics
```

---

### 🧠 Knowledge Domains

- 🛡️ Web Application Security (XSS, SQLi, CSRF, SSRF)
- 🔓 Penetration Testing (Manual & Automated)
- 🐚 Linux & Bash Scripting
- 🌐 Networking Protocols & Analysis
- 📊 Log Analysis & Incident Response
- 🔗 OSINT & Reconnaissance

---

### 🛠️ Tools of the Trade

| Recon & Scanning   | Burp Suite · Nmap · Sublist3r · Amass · Nuclei |
|--------------------|-----------------------------------------------|
| Exploitation       | Metasploit · SQLmap · XSS-Scanner · Hydra     |
| Forensics          | Autopsy · Volatility · Wireshark              |
| Scripting & Dev    | Bash · Python · JavaScript                    |
| Reporting & Docs   | Obsidian · Markdown · Ghostwriter             |

---

### 🧪 Lab Projects

- 🧬 **Cyber Range**: Created a personal vulnerable lab to simulate attacks and test defenses.
- 🔐 **HackTheBox & TryHackMe**: Active player with real-world scenario learning.
- 🔎 **Threat Intel Report**: OSINT collection and automated threat mapping.

---

### 📡 Currently Hacking Into

- 🧠 Reverse Engineering Malware
- 📱 Android Application Pentesting
- 🤖 Automating Recon with Python

---

### 🧬 GitHub Metrics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=CyberSnehal&show_icons=true&theme=radical" alt="GitHub Stats"/>
  <img src="https://camo.githubusercontent.
"/>
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=CyberSnehal&bg_color=000000&color=00ff00&line=ff0000&point=ffffff&area=true&hide_border=true" alt="Activity Graph"/>
</p>

---

### 🧭 Contact Me in the Shadows

- 🧑‍💻 GitHub: [CyberSnehal](https://github.com/SnehalPawarSec)
- 🐦 X (Twitter): [@CyberSnehal](https://twitter.com/CyberSnehal)
- 📧 Email: snehalajaypawar2@gmail.com

---

<p align="center">
  <img src="https://raw.githubusercontent.com/Aniket965/Aniket965/master/pacman.svg" width="200"/>
</p>
